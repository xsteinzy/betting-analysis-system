...
from models.nfl.config import VALUE_BET_CONFIG

logger = setup_logger('value_finder_nfl')
...
class ValueFinder:
    ...
    def _generate_reasoning(self, prediction: Dict, betting_line: float,
        ...
        # Position context
        if position:
            reasoning_parts.append(f"{position}:")
        
        # Edge explanation
        if edge > 0:
            reasoning_parts.append(
                f"Model projects {predicted_value} {prop_type}, "
                f"line is {betting_line}, giving a +{abs(edge):.1f} edge."
            )
        else:
            reasoning_parts.append(
                f"Model projects {predicted_value} {prop_type}, "
                f"line is {betting_line}, showing a -{abs(edge):.1f} disadvantage."
            )
...
def main():
    ...
    print("\n" + "="*60)
    print("VALUE BET ANALYSIS - NFL")
    print("="*60)
    print(f"Player: {over_eval['player_name']} ({over_eval['position']})")
    print(f"Week: {over_eval['week']}")
