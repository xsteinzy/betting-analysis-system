"""
NBA ML Model Training Pipeline
...
class ModelTrainer:
    ...
    def train_model_for_prop(self, prop_type: str) -> Optional[Dict[str, Any]]:
        ...
        Args:
            prop_type: Type of prop to predict (e.g., 'points', 'rebounds')
        ...
        try:
            ...
            for model_name, preds in predictions.items():
                ...
                # Calculate accuracy within X points
                within_1 = np.mean(np.abs(y_test - preds) <= 1) * 100
                within_2 = np.mean(np.abs(y_test - preds) <= 2) * 100
                within_3 = np.mean(np.abs(y_test - preds) <= 3) * 100
                ...
                logger.info(f"  Within 2 points: {within_2:.1f}%")
            ...
            within_1_ensemble = np.mean(np.abs(y_test - ensemble_predictions) <= 1) * 100
            within_2_ensemble = np.mean(np.abs(y_test - ensemble_predictions) <= 2) * 100
            within_3_ensemble = np.mean(np.abs(y_test - ensemble_predictions) <= 3) * 100
            
            results['ensemble'] = {
                'mae': float(mae_ensemble),
                'rmse': float(rmse_ensemble),
                'r2': float(r2_ensemble),
                'within_1_point': float(within_1_ensemble),
                'within_2_points': float(within_2_ensemble),
                'within_3_points': float(within_3_ensemble)
            }
            ...
            logger.info(f"  Within 1 point: {within_1_ensemble:.1f}%")
            logger.info(f"  Within 2 points: {within_2_ensemble:.1f}%")
            logger.info(f"  Within 3 points: {within_3_ensemble:.1f}%")
    ...
    def train_all_models(self, prop_types: List[str] = None) -> Dict[str, Any]:
        ...
        logger.info(f"\n{'#'*60}")
        logger.info(f"Starting NBA ML Model Training Pipeline")
        ...
        if all_results:
            ...
            logger.info(f"{'Prop Type':<20} {'Ensemble MAE':<15} {'Best Model':<20}")
            logger.info(f"{'-'*60}")
            ...
            for prop_type, result in all_results.items():
                ...
                logger.info(f"{prop_type:<20} {ensemble_mae:<15.3f} {best_model_name:<20}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train NBA ML prediction models')
    parser.add_argument('--prop-types', nargs='+', choices=PROP_TYPES,
                       help='Specific prop types to train (default: all)')
    parser.add_argument('--test', action='store_true',
                       help='Test mode: train only one prop type (points)')
    ...
    try:
        ...
        if args.test:
            logger.info("Running in TEST mode - training points only")
            prop_types = ['points']
